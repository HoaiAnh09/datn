...
export default function ProductsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const withImage = products.filter((product) => Boolean(product.imageUrl)).length;
    ...
    return [
      ...
      { label: 'Có ảnh', value: withImage, icon: ImageIcon },
      { label: 'Đang hư', value: damagedItems, icon: AlertCircle },
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl border border-[var(--page-divider)] bg-[var(--page-panel)] p-5 shadow-[var(--page-shadow-soft)] sm:p-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Quản lý thông tin, ảnh và tình trạng hàng khả dụng của từng mẫu.
          </p>
        </div>
        <Button
          onClick={() => {
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Khôi phục hàng hư</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-border bg-muted/30 p-4 text-sm">
              ...
              <p className="mt-1 text-muted-foreground">
                Đang hư: {restoreTarget?.damagedQuantity ?? 0} | Khả dụng:{' '}
